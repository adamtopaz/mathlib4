...
/-- Isomorphism between products and biproducts -/
noncomputable
abbrev Pi.isoBiproduct {C : Type _} [Category C] {α : Type _}
  (X : α → C) [HasZeroMorphisms C] [HasBiproduct X]:
    ∏ X ≅ ⨁ X :=
  limit.isLimit _ |>.conePointUniqueUpToIso (biproduct.isLimit _)

/-- When biproducts exist products and coproducts coincide, thus products have
    the same universal property as coproducts and all the morphisms associated-/
noncomputable
abbrev Pi.desc {C : Type _} [Category C] {α : Type _}
  {X : α → C} [HasZeroMorphisms C] [HasBiproduct X] {P : C} (p : ∀ (a : α), X a ⟶ P) :
    ∏ X ⟶ P :=
  (Pi.isoBiproduct _).hom ≫ biproduct.desc p

noncomputable
def Pi.ι {C : Type _} [Category C] {α : Type _}
  (X : α → C) [HasZeroMorphisms C] [HasBiproduct X] (a : α) : X a ⟶ ∏ X :=
  biproduct.ι  _ _ ≫ (Pi.isoBiproduct _).inv

/-- Composition of -/
lemma Pi.π_ι {C : Type _} [Category C] [HasZeroMorphisms C] {α : Type _} (X : α → C) (a : α) [HasBiproduct X] :
  Pi.ι X a ≫ Pi.π X a = 𝟙 _
    := by {dsimp only [Pi.ι] ; simp}

@[reassoc (attr := simp)]
lemma Pi.desc_ι {C : Type _} [Category C] {α : Type _}
  (X : α → C) [HasZeroMorphisms C] [HasBiproduct X] {Z : C} (f : (a : α) → (X a ⟶ Z)) (a) :
   Pi.ι _ a ≫ Pi.desc f = f a := by simp [Pi.desc, Pi.ι]

lemma Pi.hom_ext' {C : Type _} [Category C] {α : Type _}
  (X : α → C) [HasZeroMorphisms C] [HasBiproduct X] {Z : C} (f g : ∏ X ⟶ Z)
  (h : ∀ a, Pi.ι _ a ≫ f = Pi.ι _ a ≫ g) : f = g := by
    rw [← cancel_epi (Pi.isoBiproduct _).inv]
...
/--Isomorphism between coproducts and biproducts -/
noncomputable
